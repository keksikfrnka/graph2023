...
void TestTopologicalSortCore(httplib::Client *cli) {
  /**
   * @brief Тесты алгоритма topological_sort.
   *
  ...
  */
  ...
  for (const auto &[name, value] : cases) {
    std::cout << name << "... ";
    auto output = cli->Post("/TopologicalSort",
...
void TestTopologicalSort(httplib::Client *cli) {
    ...
    RUN_TEST_REMOTE(suite, cli, TestTopologicalSortCore);
}